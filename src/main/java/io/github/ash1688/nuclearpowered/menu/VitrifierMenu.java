...
public class VitrifierMenu extends AbstractContainerMenu {
    public static final int BUTTON_TOGGLE_AUTO_INPUT = 0;
    public static final int BUTTON_TOGGLE_AUTO_OUTPUT = 1;

    public final VitrifierBlockEntity blockEntity;
    private final Level level;
    private final ContainerData data;
    private static final int PLAYER_INV_SLOT_COUNT = 36;

    public VitrifierMenu(int id, Inventory inv, FriendlyByteBuf buf) {
        this(id, inv, resolveBlockEntity(inv, buf.readBlockPos()), new SimpleContainerData(6));
    }

    public VitrifierMenu(int id, Inventory inv, BlockEntity be, ContainerData data) {
        super(ModMenuTypes.VITRIFIER.get(), id);
        this.blockEntity = (VitrifierBlockEntity) be;
        ...
        addSlot(new SlotItemHandler(blockEntity.getItemHandlerForMenu(),
                VitrifierBlockEntity.SLOT_INPUT_WASTE, 44, 26));
        addSlot(new SlotItemHandler(blockEntity.getItemHandlerForMenu(),
                VitrifierBlockEntity.SLOT_INPUT_FRIT, 44, 44));
        addSlot(new SlotItemHandler(blockEntity.getItemHandlerForMenu(),
                VitrifierBlockEntity.SLOT_OUTPUT, 116, 35));

        addDataSlots(data);
    }

    private static VitrifierBlockEntity resolveBlockEntity(Inventory inv, BlockPos pos) {
        BlockEntity be = inv.player.level().getBlockEntity(pos);
        if (!(be instanceof VitrifierBlockEntity vit)) {
            throw new IllegalStateException("VitrifierMenu opened without a VitrifierBlockEntity at " + pos);
        }
        return vit;
    }
    ...
    public boolean stillValid(Player player) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
                player, ModBlocks.VITRIFIER.get());
    }
