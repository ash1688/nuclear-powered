...
public class ThermocoupleMenu extends AbstractContainerMenu {
    public final ThermocoupleBlockEntity blockEntity;
    private final Level level;
    private final ContainerData data;

    private static final int PLAYER_INV_SLOT_COUNT = 36;

    public ThermocoupleMenu(int id, Inventory inv, FriendlyByteBuf buf) {
        this(id, inv, resolveBlockEntity(inv, buf.readBlockPos()), new SimpleContainerData(4));
    }

    public ThermocoupleMenu(int id, Inventory inv, BlockEntity be, ContainerData data) {
        super(ModMenuTypes.THERMOCOUPLE.get(), id);
        this.blockEntity = (ThermocoupleBlockEntity) be;
    ...
    private static ThermocoupleBlockEntity resolveBlockEntity(Inventory inv, BlockPos pos) {
        BlockEntity be = inv.player.level().getBlockEntity(pos);
        if (!(be instanceof ThermocoupleBlockEntity tc)) {
            throw new IllegalStateException("ThermocoupleMenu opened without a ThermocoupleBlockEntity at " + pos);
        }
        return tc;
    }
    ...
    public boolean isConnected() { return data.get(2) != 0; }

    public int getGenerationRate() { return data.get(3); }

    // Returns 0..barHeight for the FE fill bar.
    ...
    public ItemStack quickMoveStack(Player player, int slotIndex) {
        // No machine slots — shift-click only moves inside the player's own inventory.
    ...
    public boolean stillValid(Player player) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
                player, ModBlocks.THERMOCOUPLE.get());
    }
