...
public class CladdingRecyclerMenu extends AbstractContainerMenu {
    public static final int BUTTON_TOGGLE_AUTO_INPUT = 0;
    public static final int BUTTON_TOGGLE_AUTO_OUTPUT = 1;

    public final CladdingRecyclerBlockEntity blockEntity;
    private final Level level;
    private final ContainerData data;
    private static final int PLAYER_INV_SLOT_COUNT = 36;

    public CladdingRecyclerMenu(int id, Inventory inv, FriendlyByteBuf buf) {
        this(id, inv, resolveBlockEntity(inv, buf.readBlockPos()), new SimpleContainerData(6));
    }

    public CladdingRecyclerMenu(int id, Inventory inv, BlockEntity be, ContainerData data) {
        super(ModMenuTypes.CLADDING_RECYCLER.get(), id);
        this.blockEntity = (CladdingRecyclerBlockEntity) be;
    ...
    private static CladdingRecyclerBlockEntity resolveBlockEntity(Inventory inv, BlockPos pos) {
        BlockEntity be = inv.player.level().getBlockEntity(pos);
        if (!(be instanceof CladdingRecyclerBlockEntity r)) {
            throw new IllegalStateException("CladdingRecyclerMenu opened without a CladdingRecyclerBlockEntity at " + pos);
        }
        return r;
    }
    ...
    public boolean stillValid(Player player) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
                player, ModBlocks.CLADDING_RECYCLER.get());
    }
