...
_Minecraft Mod — Design Document v4_
_Real-World Reactor Progression · Steam Age to Endgame_
_Target: Minecraft 1.20.1 (Forge 47.x)_
_v4 · R1 Implementation Alignment_

> **v4 supersedes v3.** R1 (Foundations) is now built and playable. This
> revision folds shipped numbers and mechanics into the core spec — pile
> heat model, thermo FE curve, PUREX chain FE costs, cooling pond step,
> cladding recycler side branch, battery specs, and the coolant-mode
> toggle. Later-release sections (R2–R4) are unchanged from v3.
...
- Target MC version: 1.20.1, loader **Forge 47.x** (the earlier v3 called out NeoForge tooling; the shipped R1 build is pure Forge per `mods.toml`)
...
|---|---|---|---|
| R1 — Foundations **(shipped, alpha)** | Tier 1 complete: pile, basic processing, basic reprocessing, steam-age power conversion, self-sustaining fuel cycle, milestone-based research via vanilla advancements + bundled FTB Quests chapter | First nuclear reactor, replaces coal power, begins isotope stockpile | **Delivered** |
...
### Step 0 — Cooling _(R1)_

Fresh spent rods leave the pile as `hot_spent_fuel_rod` — dangerously hot. Holding one in the selected hotbar slot burns the player (1 damage + fire every second). It must cool before it can enter the Shearer.

- **Cooling Pond** (single block, no FE). Input queue + cooling slot. Accepts hot rods from pipe/hopper or player hand. Cools each rod for **60 s** into a regular `depleted_uranium_fuel_rod`, then auto-outputs via any face. Place multiple in a row for parallel capacity.

### Step 1 — Decladding

Spent fuel is encased in metal cladding (Magnox alloy, aluminium, zircaloy). This must be removed before dissolution.

- **Shearer** _(R1 — 50 FE/tick × 200 ticks = 10 K FE per rod)_: mechanically chops fuel rods into short segments. Outputs `chopped_fuel` + `cladding_scrap`. Cladding scrap feeds the **Cladding Recycler** side branch (9 scraps + 20 K FE → 1 `fuel_rod_cladding`), which the Fuel Fabricator accepts as a drop-in for 4 iron ingots.
- Alternative (Magnox fuel only, R2+): chemical decladding in dilute caustic solution.
...
- **Dissolver** _(R1 — 150 FE/tick × 200 ticks = 30 K FE per rod; consumes 250 mB nitric acid)_: input chopped_fuel + nitric acid (bucket or pipe), outputs `dissolved_fuel` + `reactor_sludge` (small byproduct).
...
- **Extraction Column** _(R1 — 250 FE/tick × 200 ticks = 50 K FE per rod; consumes 250 mB extraction solvent)_: for R1 this machine folds Steps 3 + 4 + 5 together — input `dissolved_fuel` + extraction solvent, output `plutonium_239` + `reclaimed_uranium` (both solids) + `fission_product_stream` (goes to Cs column). The formal R2 partitioning/denitration/calciner split is deferred.

### Step 4 — Partitioning (U from Pu) — _R2+_
...
- Partitioning Chamber: input U+Pu organic + reducing agent, output separated U nitrate + Pu nitrate streams. _(R1 skips this step — the Extraction Column yields Pu-239 and reclaimed U directly.)_

### Step 5 — Conversion to Oxides — _R2+_
...
### Step 6 — Fission Product Separation
...
- **Cs Column** _(R1 — 350 FE/tick × 200 ticks = 70 K FE per rod; consumes 1 `ion_exchange_resin` per batch)_: input `fission_product_stream` + resin, output `cesium_137` + `residual_waste`.
...
- **Vitrifier** _(R1 — 400 FE/tick × 200 ticks = 80 K FE per rod; consumes 1 `glass_frit` per batch)_: input `residual_waste` + glass frit, output `vitrified_waste`.
...
|---|---|---|---|
| Basic (R1) | Pu-239, Cs-137, reclaimed U, vitrified waste, recycled cladding | Cooling Pond, Shearer, Dissolver, Extraction Column, Cs Column, Vitrifier, Cladding Recycler | Tier 2 construction |
...
### R1 chain totals _(shipped)_

- **Per-rod FE cost**: ~240 K FE (10 K + 30 K + 50 K + 70 K + 80 K across the five active machines). Cladding Recycler adds 20 K if you choose to recycle.
- **Combined draw while all five are running**: 1200 FE/tick. Deliberately above the 1024 FE/tick output of a single battery — running the full chain concurrently needs two piles, multiple batteries, or direct thermo-to-cable routing.
- **R1 reagent recipes** are placeholders crafted from vanilla materials (glass frit = sand + bone meal; ion exchange resin = slime + redstone; nitric acid bucket = water + gunpowder + glowstone; extraction solvent bucket = water + blaze powder + slime). The real chemistry production paths in the Supporting Chemistry table above are deferred to R2+.
...
|---|---|---|---|
| Thermocouples (direct heat → FE) | 10–100 % by heat zone (see curve below) | Thermocouple blocks adjacent to pile casings, cable to sinks | R1 |
| Coal Boiler → Steam Engine | Bootstrapping path before pile comes online | Coal, water loop (Infinite Water Source, Steam Pipe, Boiler, Steam Engine) | R1 |
| Steam turbine (heat → water → steam → turbine) | High (~80 %) | External water loop, boiler, turbine multiblock | R2+ |

### R1 thermocouple FE model _(shipped)_

| State | H / s drain on pile | FE output |
|---|---:|---|
| Idle (attached, no consumer this second) | −1 | buffered internally only |
| Active (FE extracted this second) | −2 | generates FE at heat-zone rate, extracts via cable |
| Coolant mode (sneak + right-click to toggle) | −5 | blocked — no FE pushed downstream |

| Heat zone | FE multiplier | FE/tick per thermo (raw `heat/10 × mul`) |
|---|---:|---:|
| < 3 K | × 0.5 | 0 → 150 |
| **3 K – 5 K** | **× 1.0** | 300 → 500 (peak at 5 K) |
| 5 K – 7 K | × 0.7 | 420 → 490 |
| 7 K + | × 0.1 | 70 → 100 (hard penalty) |

Output hard-capped at **600 FE/tick** per thermo. Internal buffer 10 K FE; cable network moves FE downstream.

### Battery _(shipped)_

| Stat | Value |
|---|---|
| Capacity | 5 M FE per block |
| I/O ceiling | 2048 FE/tick |
| Notes | Input and output share the same ceiling. Cables auto-distribute to batteries as gradient buffers. |
...
| Shearer | 50 |
| Dissolver | 150 |
| Extraction Column | 250 |
| Cs Column | 350 |
| Vitrifier | 400 |
| Cladding Recycler | 100 |
| **Entire reprocessing chain running concurrently** | **1200** |

## R1 Pile Output _(shipped, ignoring coolant-mode thermos)_

| **Config** | **Thermos** | **Heat zone** | **Thermocouple FE/tick** |
|---|---:|---|---:|
| 3×3×3 pile, small build | 10 active | 3–4 K | ~4 K |
| 3×3×3 pile, full build | 30 active | 4–5 K (sweet spot) | ~15 K |
| 3×3×3 pile, packed | 43 active | ~5 K | ~21 K |

Pile equilibrium depends on thermo activity: mostly-idle thermos pin the pile toward 7 K (low FE/tick zone), mostly-active thermos pull it into the 3–5 K sweet spot. Coolant-mode thermos do −5 H/s but output no FE — the trade is used to fine-tune heat when an all-active setup runs too hot.
...
## Tier 1 — Graphite Pile _(R1, shipped)_

The player's first reactor. A single **Graphite Pile** block at the centre of a 3×3×3 shell of **Graphite Casing** blocks, loaded with a single fuel rod at a time. Minimal automation, self-limiting through thermal throttling, produces small but critical amounts of Pu-239 and Cs-137 for tier 2 construction.

### Structure _(shipped)_

- Exactly one Graphite Pile block at the centre of a 3×3×3 shell: 26 Graphite Casings, pile at one of the four side-face-centre positions so the player can interact with it (not top/bottom).
- Structure is auto-validated every tick; breaks immediately kill any active burn.
- Single fuel slot + single depleted slot. Fuel rods stack to 64.

### Heat Model _(shipped, per second)_

| Contributor | H/s |
|---|---:|
| Fuel rod (while burning, heat < 8 K) | +30 |
| Fuel rod (heat ≥ 8 K, runaway cutoff) | 0 — rod still burns, time wasted |
| Each graphite casing, pile heat < 3 K | +5 |
| Each graphite casing, 3–4 K | +4 |
| Each graphite casing, 4–5 K | +3 |
| Each graphite casing, 5–6 K | +2 |
| Each graphite casing, 6–7 K | +1 |
| Each graphite casing, 7 K + | 0 (floor) |
| Thermocouple — idle | −1 |
| Thermocouple — active (FE drawn this second) | −2 |
| Thermocouple — coolant mode | −5 |

Max heat: 10 K safety ceiling. Passive decay handled entirely by the casing-band mechanic — no separate decay constant.

Equilibrium is dynamic: unattended piles climb to ~7 K; 30 active thermos pull it into the 3–5 K sweet spot; mixing in a handful of coolant-mode thermos lets players dial equilibrium down to ~3 K.

### Cooling Options _(shipped)_

- **Thermocouples** — placed adjacent to casings (not to the pile block itself). Produce FE (see Power Conversion Chain) and cool the pile per the table above. Up to 43 thermos fit on a 3×3×3 pile's external surface (6 faces × 9 positions − 11 reserved for pile face, depleted-rod output, and hookup gaps).
- **Heater** _(pile-age assist block)_ — optional right-click toggle. Pumps +20 heat/tick into an adjacent pile up to 1.5 K, tapers to 0 by 2.5 K. Useful for warming a cold pile without fuel waste.
- **Cooling Pond** _(see reprocessing chain)_ — sits next to the pile's depleted-rod output, accepts auto-pushed hot rods, neutralises them before they enter the shearer.

### Operation & Outputs _(shipped)_

- **Startup**: drop a uranium fuel rod into the pile's fuel slot. Burn starts immediately if the 3×3×3 shell is valid. The Heater block gives a fast warm-up boost; otherwise fuel brings heat up from 0 on its own (~100 s to 3 K via fuel alone).
- **Primary output**: heat, converted by thermocouples to FE (or, externally, by a coal boiler + steam engine for a pre-reactor power bootstrap).
- **Byproduct output**: `hot_spent_fuel_rod` — must pass through a Cooling Pond before it can be sheared. Holding a hot rod in the active hotbar slot burns the player (fire + 1 damage/sec). Cooled rods become standard `depleted_uranium_fuel_rod`, feeding the reprocessing chain.
- **Rod burn time**: 4000 game ticks (~200 s at 20 TPS). Burn is always at constant rate — no slowdown curve.
- **Steam-age accessible**: Coal Boiler + Steam Engine can produce FE without the pile being online, so players can power their first processing line before reaching uranium.
...
## Platform & Libraries _(shipped)_

- Minecraft 1.20.1
- **Forge 47.x** (v3 originally targeted NeoForge tooling; the shipped build is Forge per `mods.toml`)
- Multiblock: Custom lightweight detection on the pile's 3×3×3 shell
- Fluid handling: Forge fluid capabilities
- Quest system: Vanilla advancements for zero-dependency progression, plus a bundled FTB Quests chapter at `data/ftbquests/quests/chapters/tier1.snbt` for modpacks that ship FTB Quests
- Recipe visibility: JEI integration with custom recipe categories for Crushing / Washing / Fabricating
- Power: FE via Forge energy capability

## R1 Implementation Order _(shipped ✅ / deferred ⏳)_

### Phase 1 — Foundations ✅

- Project skeleton, Forge mod setup, tag definitions ✅
- Uranium + thorium ores (overworld / deepslate / nether / end) ✅
- Raw uranium, crushed uranium, yellowcake, uranium ingot items ✅
- Raw thorium, crushed thorium, thorium concentrate items ✅ (no consumer yet — wait for R2 thorium chain)

### Phase 2 — Basic Processing ✅

- Crusher (FE-powered, smart-output guarded) ✅
- Ore Washer (FE + water tank) ✅
- Electric Furnace (FE-powered smelting; accepts vanilla smelting recipes including custom mod ones) ✅
- Fuel Fabricator (accepts either fuel slot order; dual cladding recipes — iron or recycled) ✅

### Phase 3 — Tier 1 Pile ✅

- Graphite Pile block + Graphite Casing decorative/structural block ✅
- 3×3×3 shell validation + casing count tracking ✅
- Heat model (fuel +30 H/s, casing-band +5/+4/+3/+2/+1/0, thermo −1/−2/−5, fuel cutoff 8 K) ✅
- Heater block (+20 H/tick startup assist, ramps down 1.5 K → 2.5 K) ✅
- Hot Spent Fuel Rod item (burns player on active hotbar, cools in Cooling Pond) ✅

### Phase 4 — Power Conversion ✅

- Thermocouple block (FE generation + pile cooling per the table above, right-click GUI, sneak-right-click for coolant mode) ✅
- Coal Boiler + Steam Engine (bootstrap pre-pile power path) ✅
- Energy Cable — network / conduit model (no per-cable buffer, BFS-distributes pushes) ✅
- Steam Pipe — same network model for steam ✅
- Battery (5 M FE capacity, 2048 FE/tick I/O) ✅
- Infinite Water Source (creative/test helper) ✅

### Phase 5 — Reprocessing (PUREX) ✅

- Cooling Pond (cool hot spent rods) ✅
- Shearer (decladding) ✅
- Dissolver (chopped fuel + nitric acid → dissolved fuel + sludge) ✅
- Extraction Column (dissolved fuel + extraction solvent → Pu-239 + reclaimed U + fission product stream) ✅
- Cs Column (fission product stream + ion exchange resin → Cs-137 + residual waste) ✅
- Vitrifier (residual waste + glass frit → vitrified waste) ✅
- Cladding Recycler (9 scrap → 1 fuel_rod_cladding, feeds back into Fabricator) ✅
- Reagent recipes (placeholder vanilla-material paths) ✅

### Phase 6 — Radiation & Shielding ⏳

Deferred to a later R1 patch or rolled into R2 with the isotope economy proper. R1 items are collectible but do not yet radiate.

### Phase 7 — Decay System ⏳

Deferred. Planned for R2 to tie into the full isotope economy.

### Phase 8 — Quest Book & Polish ✅

- Vanilla advancement tree covering the tier 1 spine (12 nodes from "Nuclear Dawn" → "Self-Sustaining") ✅
- FTB Quests chapter shipped as `data/ftbquests/quests/chapters/tier1.snbt` ✅
- JEI categories for Crushing / Washing / Fabricating ✅
- Balance pass (heat model + FE curve tuned during playtests) ✅
- Custom GUI panels for the six reprocessing machines + Cooling Pond ✅
- Ghost slot icons indicating expected input items in empty slots ✅
- HD artwork for blocks and items (64×64 isometric renders, placeholder per-face cube_all) ✅

## Success Criteria for R1 Release _(shipped status)_
...
- ✅ Steam-age accessibility confirmed: Coal Boiler + Steam Engine path lets players bootstrap FE before any uranium work.
- ✅ Self-sustaining target met: a 3×3×3 pile with ~30 active thermos produces ~15 K FE/tick — the reprocessing chain draws 1.2 K FE/tick, leaving ample headroom.
- ✅ Milestone guidance: vanilla advancement tree covering the tier 1 spine (12 nodes) + bundled FTB Quests chapter for modpacks.
- ⏳ SkyGreg validation: pending real-world pack testing.
- ⏳ Radiation + decay systems: deferred from original R1 scope to a later patch / R2.
...
- ~~Exact FE output tuning to hit self-sustaining target — requires playtest~~ _Resolved: peak 500 FE/tick per thermo at 5K, chain draws 1200 FE/tick, a single pile with ~30 active thermos comfortably funds the chain + base._
- ~~Forge compatibility scope — full parity with NeoForge, or NeoForge-only for launch?~~ _Resolved: Forge-only for 1.20.1. NeoForge port deferred to post-R1._
